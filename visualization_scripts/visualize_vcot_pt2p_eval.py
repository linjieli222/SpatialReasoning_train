"""
Visualize VCoT PT2P evaluation results as interactive HTML.
Shows input images (topdown + ego views), generated sideview thought,
...
def build_html(result_xlsx, subset, output_path, max_samples=None):
    ...
    ds_lookup = {i: item for i, item in enumerate(ds)}
    ...
    for idx, row in df.iterrows():
        ...
        question_short = html_lib.escape(str(row['question'])[:300])
        ...
        for letter in ['A', 'B', 'C', 'D']:
            if letter in row and pd.notna(row[letter]):
                cls = []
                if letter == gt_letter:
                    cls.append('choice-gt')
                if letter == pred_letter:
                    cls.append('choice-pred')
                choice_text = html_lib.escape(str(row[letter]))
                choices_html += f'<span class="choice {" ".join(cls)}">{letter}: {choice_text}</span>'
        ...
        <div class="sample {status_class}">
            ...
            <div class="sample-body">
                ...
                <div class="images-section">
                    ...
                    <div class="img-row">
                        <div class="img-group"><div class="group-label">Visual Thought</div><div class="img-row">{gen_sv_html}</div></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    ckpt_name = Path(result_xlsx).parent.parent.parent.name

    full_html = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8">
<title>VCoT PT2P Eval — {ckpt_name}</title>
<style>
...
<h1>VCoT PT2P Evaluation — {ckpt_name} (Acc: {accuracy:.1f}%)</h1>
<div class="stats">
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subset', default='td_ego_dir')
    parser.add_argument('--output', required=True)
    parser.add_argument('--max_samples', type=int, default=None)
    args = parser.parse_args()
    build_html(args.result_xlsx, args.subset, args.output, args.max_samples)
